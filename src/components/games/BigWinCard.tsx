...
const BigWinCard = ({
  ...
  return (
    <div
      className={cn(
        "flex-shrink-0 w-[180px] bg-neutral-900 rounded-lg overflow-hidden hover:ring-2 hover:ring-primary/50 transition-all cursor-pointer",
        className
      )}
    >
      ...
      >
        <div className="text-white/20 text-2xl font-bold uppercase">
          {gameCategory.slice(0, 3)}
        </div>
        {/* Game title overlay */}
        <div className="absolute bottom-0 left-0 right-0 bg-black/60 px-2 py-1">
          <p className="text-white text-xs truncate">{gameTitle}</p>
        </div>
      </div>
