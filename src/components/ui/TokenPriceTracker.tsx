  ...
  return (
    ...
    >
      <div className="flex items-center">
        ...
        <span className={compact ? "text-xs font-medium" : "text-sm"}>
          {compact ? symbol : name}
        </span>
      </div>
...
const TokenPriceTracker = ({ compact = false }: { compact?: boolean }) => {
  ...
  return (
    <div className="w-full">
      {!compact && <h3 className="font-medium text-sm mb-2">Token Prices</h3>}
      <div className={cn("flex", compact ? "flex-row gap-3" : "flex-col")}>
        {isLoading
          ? Array(compact ? 3 : 5)
              .fill(null)
              .map((_, i) => (
